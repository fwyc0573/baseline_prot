...
class Channels:
    ...
    def put(self, task: CommTask, force_feature=True, overlapped=True):
        ...
        # print()
        # print("-------- complete comm task info --------")
        # print(f"task.name: {task.name}")
        # print(f"task.comm_type: {task.comm_type}")
        # print(f"task.group: {task.group}")
        # print(f"task.collective: {task.collective}")
        # print()
    ...
    def _execute(self):
        ...
        if not self.done():
            ...
            for tsk in execed_tasks:
                # print(f"Channels: tsk:{tsk}")
                global new_devices
                for dev_id in tsk.group:
                    new_devices[int(dev_id.split(':')[1])]['comm'] += tsk.cost
                    # print(f"dev_id:{dev_id}, tsk.cost:{tsk.cost}")
...
class DevExecutor:
    ...
    def execute(self, sync_clock=False):
        ...
        if not self.comp_queue.empty():
            ...
            global new_devices
            dev_id = task.rank
            new_devices[dev_id]['comp'] += task.cost
        ...
        # for dev_id, metrics in new_devices.items():
        #     prefix = "Proteus | "
        #     print(f"{prefix} DevExecutor | Device {dev_id}: comp = {metrics['comp']}, comm = {metrics['comm']}")
        return ntasks
...
class Simulator:
    ...
    def __init__(self,
        ...
        self.flexflow = FlexFlow #FlexFlow
    ...
    def run(self, profile=None, cprofile_analysis=False):
        global new_devices
        new_devices = {dev_id: {'comp': 0, 'comm': 0} for dev_id in range(self.strategy_tree.dev_topo.ndevs)}
    ...
    def run_flexflow(self):
        ...
        while len(ready_queue) > 0:
            ...
            if cur_task.type in [TaskType.TASK_COMP, TaskType.TASK_MEM]:
                ...
                duration = end_t - ready_t
                new_devices[dev_id]['comp'] += duration
                ...
                comm_time = cur_task.cost

                ready_t = cur_task.get_ready()
                for dev_id in cur_task.group:
                    ready_t = max(ready_t, devices[int(dev_id.split(':')[1])])
                    new_devices[int(dev_id.split(':')[1])]['comm'] += comm_time
                end_t = cur_task.set_start_time(ready_t)

                end_t = cur_task.set_start_time(ready_t)
                # print(f"cur_task.comm_type = {cur_task.comm_type}, duration = {end_t-ready_t}, cur_task.group={cur_task.group}")
        ...
        # new_devices = {dev_id: {'comp': 0, 'comm': 0} for dev_id in range(self.strategy_tree.dev_topo.ndevs)}
        # 请print new_devices中每个device的comp和comm的信息
        # 打印每个设备的 comp 和 comm 信息
        # for dev_id, metrics in new_devices.items():
        #     print(f"Flex flow -> Device {dev_id}: comp = {metrics['comp']}, comm = {metrics['comm']}")


    def print_stats(self):
        global new_devices 
        for dev_id, metrics in new_devices.items():
            prefix = "Flexflow | " if self.flexflow else "Proteus | "
            print(f"{prefix}Device {dev_id}: comp = {metrics['comp']}, comm = {metrics['comm']}")
