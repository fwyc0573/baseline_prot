...
class ProfileWrapper(object):
    ...
    def __init__(self, func, warmup_iters=10):
        super().__init__()
        self.func = func
...
def make_randn_label(size, dev_type, nclasses):
    #x = torch.randint(0, num_embeddings, (batch_size, seq_length), dtype=torch.int64, device=dev.type)
...
def profile_crossentropy(op, dev):
    ...
    target = make_randn_label(op.ins[1], dev.type, x.size(-1))
    if op.loss_type == 'mse':
        target = target.float()
...
#             # 生成索引的数量
#             if num_indices_per_lookup_fixed:
#                 sparse_group_size = min(num_indices_per_lookup, size)  # 确保数量不会超过size
#             else:
#                 r = np.random.random(1)
#                 sparse_group_size = np.int64(
#                     np.round(max([1.0], r * min(size, num_indices_per_lookup)))
...
#                     rand_data_mu = (rand_data_max + rand_data_min) / 2.0
#                 r = np.random.normal(rand_data_mu, rand_data_sigma, sparse_group_size)
#                 sparse_group = np.clip(r, 0, size - 1)  # 使用clip确保索引合法
#                 sparse_group = np.unique(sparse_group).astype(np.int64)
#             elif rand_data_dist == "uniform":
#                 sparse_group = np.random.randint(0, size, sparse_group_size, dtype=np.int64)  # 使用更直接的randint生成合法索引
#             else:
#                 raise ValueError(f"{rand_data_dist} distribution is not supported. \
#                                   Please select 'uniform' or 'gaussian'")
...
def profile_embedding(op, dev):
    if op.attr is None:
        ...
        print("Generated indices tensor:", x)
        print("Embedding layer number of embeddings:", layer.num_embeddings)
        ...
        for idx_tensor in x[1]:
            assert torch.all(idx_tensor >= 0) and torch.all(idx_tensor < op.ins[1][0]), "Generated indices are out of bounds"
...
#         # 检查索引的范围，确保合法性
#         assert torch.all(x >= 0) and torch.all(x < layer.num_embeddings), "Generated indices are out of bounds"
...
#         # 对所有生成的索引进行检查
#         for idx_tensor in x[1]:
#             print("Index tensor:", idx_tensor)  # 输出调试信息
#             assert torch.all(idx_tensor >= 0) and torch.all(idx_tensor < op.ins[1][0]), "Generated indices are out of bounds"
