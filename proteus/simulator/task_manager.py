...
class TaskManager(object):
    ...
    def get_schedule(self):
        ...
        while not task_queue.empty():
            ...
            if isinstance(task, CommTask):
                self.comm_time += (task.end - task.start)
            elif isinstance(task, CompTask):
                self.comp_time += (task.end - task.start)
            elif isinstance(task, MemTask):
                self.mem_time += (task.end - task.start)
    ...
    def print_task_times(self):
        print(f"Comm Task Time: {self.comm_time} ms")
        print(f"Comp Task Time: {self.comp_time} ms")
        print(f"Mem Task Time: {self.mem_time} ms")