...
cluster=clusters/h800/n1_g8.json
PP=8
TP=1
DP=$((${GPU_NUM}/${TP}/${PP}))
...
output_file="./log_test/new_${MODE}_GPU${GPU_NUM}_PP${PP}_TP${TP}_MICBATCH${NUM_MICBATCH}_BATCHSIZE${MICRO_BATCH_SIZE}_MODEL${MODEL_SIZE}.txt"

    # --reprofile \

## NCCL_DEBUG=INFO NCCL_DEBUG_SUBSYS=GRAPH \
mpirun -np 1 python megatron_gpt.py -model gpt \
    -global-bs $GLOBAL_BATCH_SZIE -n-macro-batch $NUM_MICBATCH \
