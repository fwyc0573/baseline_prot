  ...
  for (int a=0; a<nAlgos; a++) {
    ...
    for (int p=0; p<NCCL_NUM_PROTOCOLS; p++) {
      ...
      if (time >= 0 && time < minTime) {
        ...
        std::cout << "Algorithm: " << info->algorithm << ", Protocol: " << info->protocol
                  << ", MinTime: " << minTime << ", Ret[0]: " << ret[0] << ", Ret[1]: " << ret[1] << std::endl;
      }
    }
  }
...
std::vector<float> Communicator::broadcast(size_t nBytes, int root){
  ...
  std::vector<float> result = getAlgoInfo(&info, 0, 1);

  // 在这里更新 algorithm_ 和 protocol_
  this->algorithm_ = info.algorithm;
  this->protocol_ = info.protocol;
...
//       .def_property_readonly("algorithm", [](const Communicator &c){ return c.algorithm_; }) // 暴露 algorithm
//       .def_property_readonly("protocol", [](const Communicator &c){ return c.protocol_; }); // 暴露 protocol
// }
PYBIND11_MODULE(binding, m)
{
  py::class_<Communicator> communicator(m, "Communicator");
  communicator.def(py::init<std::vector<std::vector<int>>,
      ...
      .def("get_graph_type_inter", &Communicator::get_graph_type_inter)
      .def("get_cross_node", &Communicator::get_cross_node)
      .def_property_readonly("algorithm", [](const Communicator &c)
                             { return c.algorithm_; }) // 注意这里的分号 ';' 结束 .def() 链
      .def_property_readonly("protocol", [](const Communicator &c)
                             { return c.protocol_; }); // 注意这里的分号 ';' 结束 .def() 链
}